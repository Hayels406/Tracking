...
videoLocation = '/data/b1033128/Videos/CaseH1.mov'
save = '/data/b1033128/Tracking/CaseH1/'
...
cropVector = [300,400,1100,900]
quadCrop   = [1100,800,1300,1000]
fixedCrop = [1700,200,1900,400]
...
print 'Skipping first 18 frames while the sheep are out of view'
while(frameID <= 18):
    ret, frame = cap.read()
    frameID += 1
if frameID > 0:
...
while(frameID <= runUntil):
    ...
    if ret == True:
        ...
        filtered = cv2.erode(filtered,np.ones((3,3),np.uint8),iterations = 3)
        ...
        minPixels = 15#np.min([np.max([50, smallSheep]), 100])
        ...
        for label in np.unique(labels):
            ...
            if numPixels > minPixels:
                ...
                if frameID == 0:
                    ...
                    dilation = cv2.dilate(labelMask,np.ones((3,3),np.uint8),iterations = 3)
                    prev,Ids = getPredictedID(lastT, dilation, cropVector, rectangle)
                    k = len(Ids)
                    if k == 0:
                        continue
