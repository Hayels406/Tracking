...
videoLocation = '/data/b1033128/Videos/CaseJ.mov'
save = '/data/b1033128/Tracking/CaseJ1/'
...
cropVector = [1500,0,2300,600]
quadCrop   = [1650,0,1850,50]
#bsCrop = [375,725,475,825]
fixedCrop = [1400,1000,1500,1100]
...
print 'Skipping first 15 frames while the sheep are out of view'
while(frameID <= 15):
    ret, frame = cap.read()
    frameID += 1
if frameID > 0:
...
if restart >= 0:
    ...
    quadLocation, _ = np.load(save+'quad'+str(restart)+'.npy')
...
while(frameID <= runUntil):
    ...
    if ret == True:
        ...
        if frameID >= 80:
            quadLocation, quadCrop = getQuad(full, quadLocation, quadCrop, quadDark, frameID-80)
        ...
        for label in np.unique(labels):
            ...
            if numPixels > minPixels:
                ...
                if frameID == 0:
                    ...
                    lastT = np.array(prediction_Objects)
                    dilation = cv2.dilate(labelMask,np.ones((5,5),np.uint8),iterations = 2)
                    prev,Ids = getPredictedID(lastT, dilation, cropVector, rectangle)
                    k = len(Ids)
                    if k == 0:
