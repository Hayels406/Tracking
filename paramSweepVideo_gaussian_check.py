...
os.system('mkdir -p ' + save+'ParamSweep/gaussianL'+str(lG))
os.system('mkdir -p ' + save+'ParamSweep/gaussianL'+str(lG)+'/sigma' +l1+'-'+l2)

plt.close('all')
plt.imshow(filtered, cmap = 'gray')
plt.gca().set_aspect('equal')
plt.gca().set_axis_off()
plt.savefig(save+'ParamSweep/gaussianL'+str(lG)+'/sigma' +l1+'-'+l2+'/filtered'+str(frameID+skip+19).zfill(4), bbox_inches='tight')
...
plt.gca().set_aspect('equal')
plt.gca().set_axis_off()
plt.savefig(save+'ParamSweep/gaussianL'+str(lG)+'/sigma' +l1+'-'+l2+'/'+str(frameID+skip+19-1).zfill(4), bbox_inches='tight')

np.savetxt(save+'ParamSweep/gaussianL'+str(lG)+'/sigma' +l1+'-'+l2+'/sheepDetected.csv', [int(l)], fmt='%i')
